...
export class GameFlagBoxComponent {
  ...
  ngAfterViewInit(): void {    
    if(this.currFile == ""){
      this.currFile = this.pickFlag();
    } else{
      console.log("hello" + this.currFile)
    }
  }

  pickFlag(): string{
    const num: number = Math.floor(Math.random() * (this.fileNames.length - 0 + 1)) + 0;
    return "./assets/svg/" + this.fileNames[num];
